...
export default function (input) {
  ...
  for (let j = 0; j < cells[0].length; j++) {
    max = Math.max(max, count(cells, [0, j, "down"]));
    max = Math.max(max, count(cells, [cells.length - 1, j, "up"]));
  }
  for (let i = 0; i < cells.length; i++) {
    max = Math.max(max, count(cells, [i, 0, "right"]));
    max = Math.max(max, count(cells, [i, cells[0].length - 1, "left"]));
  }
