...
describe("day", () => {
  test.only("part1", () => {
    ...
    expect(part1(file)).toBe(405);
    expect(part1(realInput)).toBe(29130);

    const answer = part1(realInput);

    // const answerFile = readFileContent(path.resolve(__dirname, "./ans.txt"))
    //   .split("\n")
    //   .map((i) => parseInt(i));

    // const realInputs = realInput.split("\n\n");
    // for (let i = 0; i < realInputs.length; i++) {
    //   const inp = realInputs[i];

    //   console.log(inp);
    //   expect(part1(inp)).toBe(answerFile[i]);
    // }
